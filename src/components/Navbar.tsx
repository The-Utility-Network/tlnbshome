...
const navLinks = [
    ...
    { label: 'COLLECTIONS', href: '#perennial-waters' },
    { label: 'FOUNDERS', href: '#founders' },
];

const externalLinks: { label: string; href: string; internal?: boolean }[] = [
    { label: 'BLOOM', href: 'https://medium.com/@theutilityco' },
...
export default function Navbar({ themeColor = '#10b981' }: NavbarProps) {
            ...
            >
                <div className="max-w-7xl mx-auto px-6 flex items-center justify-between">
                        ...
                        <Link href="/" className="flex items-center gap-3 group">
                            <div className="w-10 h-10 relative">
                                <Image
                                    ...
                                    alt="TLNBS Logo"
                                    fill
                                    className="object-contain drop-shadow-[0_0_8px_rgba(16,185,129,0.3)] group-hover:scale-110 transition-transform duration-500"
                                />
                            </div>
                        ...
                        <div className="hidden lg:flex items-center gap-1">
                            {navLinks.map((link) => (
                                <Link
                                    ...
                                    className="px-4 py-2 text-[10px] font-mono tracking-widest text-emerald-100/40 hover:text-emerald-400 hover:bg-white/5 rounded-lg transition-all duration-200"
                                >
                                    {link.label}
                                </Link>
                            ))}
                    ...
                    <div className="flex items-center gap-4">
                            ...
                            {externalLinks.map((link) => (
                                link.internal ? (
                                    <Link
                                        key={link.label}
                                        href={link.href}
                                        className="px-3 py-1.5 text-[10px] font-mono tracking-widest text-emerald-100/30 hover:text-white border border-white/5 hover:border-emerald-500/30 rounded-full transition-all duration-200"
                        ...
                        {/* CTA Button */}
                        <Link
                            href="https://pwc.thelochnessbotanicalsociety.com"
